...
import { FormComponent } from '@components/dashboard/ticket-promotions/form/form.component';

@Component({
  selector: 'app-ticket-promotions',
  templateUrl: './ticket-promotions.component.html',
  styleUrls: ['./ticket-promotions.component.scss']
})
export class TicketPromotionsComponent implements OnInit {
  ...
  getPromotions() {
    ...
    this.promotionService.getTicketPromotions({ page: this.currentPage, per_page: 20, ...values }).subscribe(
      (res) => { 
        this.promotions = res 
        this.loading = false;
      });
